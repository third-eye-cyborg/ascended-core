...
`@third-eye-cyborg/contracts` defines platform-neutral domain types organized into
...
All contracts build on `@third-eye-cyborg/core`:
...
import { isPost, ContentVisibility, type Post } from "@third-eye-cyborg/contracts";
...
- **`CommunityEvent`** + **`EventKind`**, **`Rsvp`** + **`RsvpStatus`**,
  **`LiveSession`**. (Distinct from the *domain-event bus* in
  `@third-eye-cyborg/events` — this context models user-facing scheduled events.)
...
- **`AuditEvent`** + `isAuditEvent` — a neutral audit record shape. Audit
  *storage* is a provider port (`@third-eye-cyborg/providers`), and audit trails contain
  no secrets or PII.
