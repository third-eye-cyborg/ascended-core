...
`@ascended/privacy` makes privacy a **first-class routing concern**. Before any
provider call is dispatched (especially AI calls routed by
`@ascended/ai-router`), a `PrivacyPolicyEnforcer` decides whether it is allowed
...
`PrivacyMode` (from `@ascended/privacy`):
...
```ts
import { PrivacyBlockedError } from "@ascended/privacy";
