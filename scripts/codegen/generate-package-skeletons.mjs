...
function pkgJson(name, cfg, isExample) {
  ...
  const pkg = {
    name: isExample ? `@ascended/example-${name}` : `@ascended/${name}`,
    version: "0.1.0",
    ...
    dependencies: Object.fromEntries([
      ...cfg.deps.map((d) => [
        isExample ? (Object.keys(EXAMPLES).includes(d) ? `@ascended/example-${d}` : `@ascended/${d}`) : `@ascended/${d}`,
        "workspace:*",
      ]),
...
function tsconfig(name, cfg, isExample) {
  ...
  const paths = Object.fromEntries(
    cfg.deps.map((d) => {
      const pkgName = Object.keys(EXAMPLES).includes(d) ? `@ascended/example-${d}` : `@ascended/${d}`;
